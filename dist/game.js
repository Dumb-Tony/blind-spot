...
(function(){
  ...
  function loadProgress(){try{const data=JSON.parse(storage.get('blindspot-overhaul-progress'));if(!data||typeof data!=='object')throw 0;return{stars:Array.from({length:LEVELS.length},(_,i)=>Math.max(0,Math.min(3,Number(data.stars?.[i])||0))),last:Math.max(0,Math.min(LEVELS.length-1,Number(data.last)||0))}}catch(_){return{stars:Array(LEVELS.length).fill(0),last:0}}}
