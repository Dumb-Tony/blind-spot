(function(global){
  ...
  class Renderer{
    ...
    rebel(sim,front=false){
      const rig=global.BlindSpotRebelRig;let p=this.rebelPose;if(!front||!p){p=rig.stabilize(rig.pose(sim,this.shotAge,this.releasePoint,this.time,this.reduceMotion,this.readyAge),p,this.poseDt||1/60,sim?.state==='aiming'||this.shotAge<.04);this.rebelPose=p;}if(this.cohesiveTexture)global.BlindSpotPaintedSkin.drawCohesive(this.ctx,p,this.cohesiveTexture,front);else if(this.paintedTexture)global.BlindSpotPaintedSkin.draw(this.ctx,p,this.paintedTexture,front);else rig.draw(this.ctx,p,this.time,this.reduceMotion,front);
